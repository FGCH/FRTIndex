/*
FRT/Debt regressions, including ESM Rules dummy
August 2016
*/
...
regsave using "tables/reviewer_suggestions/FRT_1_esm.dta", detail(all) replace table(nonInteractSpread, ///
...
regsave using "tables/reviewer_suggestions/FRT_2_esm.dta", detail(all) replace table(nonInteractCov, ///
        order(regvars r2) format(%5.2f) paren(stderr) asterisk())
...
regsave using "tables/reviewer_suggestions/FRT_3_esm.dta", detail(all) replace table(frt1, ///
        order(regvars r2) format(%5.2f) paren(stderr) asterisk())
...
regsave using "tables/reviewer_suggestions/FRT_4_esm.dta", detail(all) replace table(frt2, ///
        order(regvars r2) format(%5.2f) paren(stderr) asterisk())

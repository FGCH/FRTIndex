...
# Raw tables created with reviewer_suggestions/frt_elections.do in Stata 12.1
...
filesFRT <- AllFiles[grep('^FRT_[1-4]_elections', AllFiles)]
...
#### FRT Interacted ####
outputFRT <- CombineFiles(filesFRT, start = 'FRT_1_elections.dta')
...
# Output
tableFRT_elections <- xtable(outputFRT, dcolumn = TRUE, booktabs = TRUE)
align(tableFRT_elections) <- 'llp{3cm}p{3cm}p{3cm}p{3cm}'
print(tableFRT_elections, include.rownames = FALSE, floating = FALSE, size = 'tiny',
      file = 'tables/frt_bond_results_elections.tex')

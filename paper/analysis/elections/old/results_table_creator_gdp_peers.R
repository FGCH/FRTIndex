################################################################################
# Create LaTeX tables from Stata output for models with GDP peer effects
# Christopher Gandrud
# MIT License
################################################################################
...
filesFRT <- AllFiles[grep('^FRT_[1-4]_gdp_peer', AllFiles)]
...
CleanUp <- data.frame(
    from = c('^.*?_stderr', '_coef', '_cons', 'N_clust', 'N$', 'r2_a',
             ...
             'l_uds', 'd_uds',
             'l_sp_wghts_pcgdp2005l_bnd_sprd_', 'd_sp_wghts_pcgdp2005l_bnd_sprd_',
             'l_sp_wghts_pcgdp2005l_lt_rtcv_f', 'd_sp_wghts_pcgdp2005l_lt_rtcv_f'
    ),
    to = c('', '', 'Constant', 'Countries', 'Observations', 'Adjusted R-squared',
           ...
           'Per Capita GDP Peers (Spreads)$_{t-1}$', '$\\\\Delta$ Per Capita GDP Peers (Spreads)',
           'Per Capita GDP Peers (LT rate COV)$_{t-1}$', '$\\\\Delta$ Per Capita GDP Peers (LT rate COV)'
    )
)


#### FRT Interacted ####
outputFRT <- CombineFiles(filesFRT, start = 'FRT_1_gdp_peer.dta')
...
print(tableFRT_gdp_peers, include.rownames = FALSE, floating = FALSE, size = 'tiny',
      file = 'tables/frt_bond_results_gdp_peers.tex')
